...
##Uses one-component dose-effect relations (OCDER) & parameters from Cucinotta1 et al. (2013). Space Radiation Cancer Risk Projections and
##Uncertainties - 2012. ##NASA Center for AeroSpace Information, Hanover, MD; http://ston.jsc.nasa.gov/collections/TRS
##User adjustable parameters are those on lines 168-172 and limits on x and y axes for some of the graphs.RKS numbers will need changing
##Main outputs are predictions E_A and E_I with 95% CI, and some graphs, for effect of mixtures of ions (fully ionized atomic nuclei)
library(ggplot2); library(deSolve)
rm(list=ls()) 
#next line has Cucinotta et al. (2013) Section 5.4 table 5.4 parameters for ions
...
lamd=function(L, param) {with(param,lamd0+lamd1*L*exp(-lamd2*L))} #Bon4 why error argument here and not in alph or kappa??
#RKS lamd interpreted as giving a cell killing modulation of LQ OCDER (one component dose effect relation)
kappa=function(L,param){with(param, (k1*L)*exp(-k2*L))}#RKS interpreted as NTE, important near d=0; approximated as initial value.
#BON4: In the above are error=0, random=0 implied defaults?RKS
#BON4: next is function to calculate effect T for a single radiation; if Eff=1, T=E_single; if Eff=2, T=E_single-NT, where NT=kappa*exp(-lamd*dose). 
#BON4: Importantly, for most of the program we work only with T_single; only near the end do we add back a kappa term 
# Uses proton=0 for Z>2; proton=1 for H, He.
T_single = function(dose, L, Eff, proton, param = parameter[[Eff]]){ #Eff: 1=TE=targeted effect; 2=NTE=non-targeted (bystander) effect. RKS added 
  beta = param$beta #initialize to be beta=0 with Z>2 RKS, i.e near d=0 TE effect is linear no threshhold (LNT).
  if(proton == 1){ beta= param$beta_p} #ions with Z<3 are approximated as LQ (linear quadratic) near d=0, RKS
  P_E = (alph(L,param)*dose + beta*(dose^2))*exp(-lamd(L,param)*dose) #LQ or LNT modulated by factor controversially attributed to cell killing RKS
  return(P_E)
}
...
E_single = function(dose, L, Eff, proton, param = parameter[[Eff]]){  
  ...
  if (Eff==2){
    P_E = P_E + kappa(L,param)*exp(-lamd(L,param)*dose)
  }
  return(P_E)
}

#a function to find the maximum of T and dose where it occurs. Each T has a maximum, often around dose=1 Gy. RKS
bound_Ed = function(L,Eff,proton, param){##
  E = c(); D = c(); ##
  for(ll in 1:length(L)){#when this function is used, L will be a vector of the LETs of the components of a mixed-ion beam
    p = proton[ll]; l = L[ll]; 
    b = param[[ll]]$beta; a = alph(l,param[[ll]]); ld = lamd(l,param[[ll]])
    if(p == 1){ b= param[[ll]]$beta_p}
    if(b !=0){
      d = (sqrt((a*ld)^2+4*b^2)-a*ld+2*b)/(2*b*ld);#the derivative of the OCDER is zero at maximum, occuring at this dose d RKS
    }else{d = 1/(ld)} 
    D = c(D,d)
    E = c(E,solve_d(d,l,Eff,p,0, param[[ll]]))#BON4: why do you need to use solve_d here?? You already know d as above
  }
  upper_E = min(E)#the smallest of the maxima in a mixed beam, used only in calculating E_B, which in turn is deemphasized.
  max_d = D[which(E==upper_E)]
  return(list(upper_E,max_d,D))
}

#equation to get dose from L,Eff,proton, and T by setting eq=0 and solving for dose#RKS treats T as independent and d as dependent variable
#instead of vice versa as in RBE calculations. #will be used in dose_E below, which is then used several times
solve_d=function(dose,L,Eff,proton,E, param = parameter[[Eff]]) {
  eq = T_single(dose,L,Eff,proton,param)-E
  return(eq)
}

gaussian_param = function(Eff){#BON4: these are gamma functions, not gaussian; please correct terminology then check script still works.
  param = parameter[[Eff]]
...
#d = f_j(E_j) inverse function E_j to d_j where j labels the jth ion in a mixture RKS
...
#first derivative of function f evaluated at E
...
sum_rE = function(E, param){#right hand side of the E_I ode. RKS
  output = c()
  for(j in 1:length(E)){
    cumrE = 0
    for(i in 1:nL){
...
solve_ode <- function(t, state, parameters) {
...
E_I <- function(state,doses,L, Eff, proton, param){
  output = ode(y = state, times = doses, func = solve_ode, parms = param)
  result = output[,2]
  if(Eff ==2){
    maxx=which.max(kappa(L,param[[1]]))
    result = result + kappa(L[maxx],param[[1]])}
  return(result)
}

#####USER-DEFINED PARAMETERS FOR MIXTURE CALCULATIONS#####
r=.02*c(9,8,7,6,5,5,4,3,2,1); L=25*(1:10); proton = rep(0,10); Eff=2; N = 50 #N >= 50 to get 95% CI
#r=c(.8,.2); L=c(.4,190); proton = c(1,0); Eff=1; N = 100 #use bigger N  (e.g. N=10000)for more accurate CI
#r=mixture proportions (must sum to 1); L=LETs; proton=1 for H, He, and proton =0 for heavier ions (0 means LQ beta is 0). 
nL = length(L); end =1.5; by = 0.02; cols = end/by+1; doses <- seq(0, end, by = by); #RKS to increase smoothness decrease "by"

############E_A with central values of the paameters##############
E_A_mean = E_A(doses, L ,Eff,proton, gen_param(Eff,nL,0))##BON4 Now deal with kappa term

############E_I with central values of the parameters###############
state <- c(E = 0);
E_I_mean = E_I(state,doses, L, Eff, proton,gen_param(Eff,nL,0))

#E_B# Demphasized because numerically similar to E_I with smaller domain of validity and conceptual flaws
...
# for(i in 1:N){
#   #print(i) #BON5 not needed here
#   E_A_mat[i,] = E_A(doses, L, Eff,proton, gen_param(Eff,nL,1))
# }
# E_A_sd = apply(E_A_mat,2,sd); E_A_sort = apply(E_A_mat,2,sort,decreasing=F); 
...
# }
# E_I_sd = apply(E_I_mat,2,sd); E_I_sort = apply(E_I_mat,2,sort,decreasing=F); 
# E_I_low = E_I_sort[floor(N*0.025),]; E_I_high = E_I_sort[ceiling(N*0.975),]; 

##############################################
####################GRAPHS####################Comment out unneeded ones
##############################################BON5 I switched blue and red in the ggplot, for lines and for ribbons
#OCDER Plot. Allows adjustment of upper dose limit.
xstart = 0; xend = 1.5; ystart= 0; yend = 40
...
##BON5: when I export the ggplot from RStudio to filename.eps the ribbons are missing enirely. I need high resolution .eps for the whole thing.
##BON5: there may be a solution. e.g. googling "r export ggplot to .eps" gave http://www.r-bloggers.com/high-resolution-figures-in-r/
##BON5: otherwise I can use plot() and lines() as shown below the ggplot; that works but is not beautiful.
...
#df= data.frame(md = doses, EI=E_I_mean, EIsd = E_I_sd, EA=E_A_mean, EAsd = E_A_sd)  #dev.off()
######Beautiful Plots###### However exporting them to .eps files gives lots of problems
#with 2.5%top and 2.5% bottom# 
...
#####Regular error bar Plots##### BON 5. Same content as ggplot; clumsier and uglier but more readily exportable to .eps high resolution
...
###2) plot E_I, E_A, and E_B together in one graph for the central parameters, without error bars
xstart = 0; xend =1.5; ystart= 0; yend = 70
plot(c(xstart,xend),c(ystart,yend),type='n',bty="l", ylab = 'E', xlab = 'd')
lines(doses,E_I_mean,col='red')
lines(doses,E_A_mean,col='blue')
#lines(dose_EB,E_B_mean,col='black')# so close to E_I it is confusing in graphs.

#Some extra output for a table 
print(1)
y=max(E_B_mean); x=max(dose_EB); z=min(which(doses>x))
compare=.5*(E_I_mean[z]+E_I_mean[z-1])
print(c(x,compare, y))
print(c(E_A_mean[cols],E_I_mean[cols]))
