...
E_single = function(dose, L, Eff, proton, param = parameter[[Eff]]){#if Eff=1, E_single=T; #if Eff=2, E_single=T+NT, where NT=kappa*exp(-lamd*dose).  
  ...
  if (Eff==2){#phi is adjustable parameter for mGy range NTE1; adjust near line 170
    P_E = P_E +(1-exp(-150*phi*dose/L))*kappa(L,param)*exp(-lamd(L,param)*dose)#NOTE 2#P_E here is sum of targeted and non-targeted effects
  }
  return(P_E)
}## 
...
#R function that can generate N sets of parameters for Monte Carlo estimates of 95% Confidence Intervals (CI)
...
E_A <- function(doses, L, Eff, proton, param = parameter[[Eff]]){
  ...
  for(l in 1:nL){
    output = output + E_single(r[l]*doses,L[l],Eff,proton[l],param[[l]])#NOTE 4
  }
  return(output)
}
...
sum_rE = function(E, param){#can be used to calculate mixture slope for integrating E_I=I(d); add IDER slopes for given mixture effect E=E_I vector
  ...
  for(j in 1:length(E)){
    ...
    for(i in 1:nL){
      ...
      temp = list(); temp[[1]] = param[[i]]   #explain why param[[i]], not [[Eff]]
...
#end=largest dose in Gy; N=# of Monte-Carlo simulations. Examples follow
#r=c(.5,.5);L=c(76,175);proton=c(0,0);Eff=2;N=50;end=.5 #2 HZE ions. Use bigger N  (e.g. N=10000) for more accurate CI
#r=c(.70,.30);L=c(170,190);proton=c(0,0);Eff=2;N=50;end=.5
...
#r=c(.4,.4,.2); L=c(50,50,200); proton = c(0,0,0); Eff=2; N = 100; end=1 #test obedience to sham mixture principle
...
r=c(.70,.30);L=c(60,190);proton=c(0,0);Eff=2;N=50;end=.5# Next are examples for the MS (Table 4).#N=50 instead of N>=1000 for speed.
...
#r=.01*c(28,20,20,12,12,4,4); L=c(60,80,35*(3:7)); proton=rep(0,7);Eff=1;N=1000;end=.5 #F and F*
...
by = 0.005#to increase smoothness decrease "by"
