...
##User adjustable parameters are near line 170 (also limits on x and y axes for some of the graphs).
##Gives main calculations for Siranart, Cheng, Handa, Sachs in preparation for Rad. Res. 
##Main outputs: predictions E_A=(S(d) if Eff=1 or S*(d) if Eff=2) and E_I=I(d) with 95% CI, with graphs, for effect of mixed ion beams
...
###FUNCTIONS### Used later after user-defined parameters have been set near line 170#Next is function to calculate targeted part T of the effect of an individual radiation (Cucinotta et al. 2013); if Eff=1, T=E_single; 
...
bound_Ed = function(L,Eff,proton, param){##
  ...
  upper_E = min(E)#smallest of the maxima in a mixed beam, used only in calculating E_B, needed only in the supplementary Information.
...
#Next is function to get simple effect additivity default or its modification prediction E_A=S(d) if Eff=1 or S*(d) if Eff=2
E_A <- function(dose, L, Eff, proton, param = parameter[[Eff]]){#use T (i.e subtract out kappa term), manipulate, later add biggest back in.
  ...
  if (Eff==2){
    ...
    output = output + kappa(L[maxx],param[[l]])*exp(-lamd(L[maxx],param[[l]])*dose)
  }
  return(output)
}

#Next is function to get linear isobole default prediction; used only in Supplementary Information
# E_B <- function(param){
...
solve_ode <- function(t, state, parameters) {#R function used when integrating the ordinary differential eq. for I(d)=E_I
...
#r=c(.8,.2); L=c(.4,190); proton = c(1,0); Eff=1; N = 10000 #use bigger N  (e.g. N=10000)for more accurate CI
#r=mixture proportions (must sum to 1); L=LETs; proton=1 for H, He, and proton =0 for heavier ions (0 means LQ beta is 0). 
nL = length(L); end =1.5; by = 0.02; cols = end/by+1; doses <- seq(0, end, by = by); #to increase smoothness decrease "by"
...
#E_B Numerically very similar to E_I. Has smaller domain of validity & well known conceptual flaws. Used only in Supplementary Information
...
#x=1.498;z=min(which(doses>x)); u=min(which(dose_EB>x));compare=.5*(E_I_mean[z]+E_I_mean[z-1]);y=E_B_mean[u] #
...
## plot E_I=I(d) and E_A=S(d) if Eff=1 or S*(d) if Eff=2 together, with error bars, allowing the user to adjust the upper dose limit (<2 Gy; if no protons <1)
...
#####Regular error bar Plots##### Same content as ggplot; clumsier, uglier, and now obsolete.
...
lines(doses,E_I_mean,col='red')
lines(doses,E_A_mean,col='blue')
#lines(dose_EB,E_B_mean,col='black')#E_B is almost always so close to E_I it is confusing in graphs.
